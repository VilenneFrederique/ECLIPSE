...
from specclust import cluster_latents, score_clusters
from specclust import modelhub
...
def test_env_dir_resolution(tmp_path, monkeypatch):
    # Place a file where SPECCLUST_MODEL_DIR points; it should resolve without download.
    fname = modelhub.REGISTRY["encoder-config"]["filename"]
    (tmp_path / fname).write_text("{}")
    monkeypatch.setenv("SPECCLUST_MODEL_DIR", str(tmp_path))
...
def test_missing_url_raises(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("SPECCLUST_MODEL_DIR", raising=False)
